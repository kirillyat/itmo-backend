...
async def prometheus_middleware(request: Request, call_next):
    ...
    # Start the timer for the latency calculation
    with REQUEST_LATENCY.labels(endpoint).time():
        response = await call_next(request)

    # Logging count of requests by method and endpoint
...
@app.get("/metrics")
async def metrics():
    return StarletteResponse(generate_latest(REGISTRY), media_type="text/plain")
